...
public class ExtensionArm : MonoBehaviour
{
    ...
    public float increment = 0.02f; //Change the value of increment the adjust the speed of the movement of the arm.
    private float count;
    ...
    public void ExtendArm(float speed)
    {
        ...
        else
        {
            if (count < (maxCount * 2))
            {
                ...
                count += speed;
                //Debug.Log("Extending: Arm-3, " + count);
            }
            else if (count < (maxCount * 3))
            {
                arm2.transform.localPosition += mvmt;
                count += speed;
                //Debug.Log("Extending: Arm-2, " + count);
            }
        }
    }
